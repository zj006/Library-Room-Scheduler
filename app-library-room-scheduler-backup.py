...
ui.add_head_html('<style>body { background-color: #f5f5f5; }</style>', shared=True)


def render_header():
    """Centered logo header for pre-auth pages (login, register)."""
    with ui.column().classes("items-center w-full mt-10 mb-4"):
        ui.label("Library Room Scheduler").classes("text-h5 font-bold text-red-700 tracking-tight")
        ui.label("Reserve study spaces easily").classes("text-sm text-gray-500")


def render_nav_bar():
    with ui.row().classes("w-full items-center justify-between px-6 py-3 bg-red-700 text-white shadow-md mb-6"):
        ui.label("Library Room Scheduler").classes("text-base font-bold tracking-wide")
        with ui.row().classes("items-center gap-4"):
            name = app.storage.user.get('name', '')
            role = app.storage.user.get('role_name', '')
            ui.label(f"{name}  ({role})").classes("text-sm opacity-90")
            ui.button("Log Out", on_click=do_logout).props("flat color=white").classes("text-sm")
...
def login_page():
    ...
    with ui.card().classes("w-full max-w-sm mx-auto p-6 shadow-md"):
        ui.label("Sign In").classes("text-lg font-bold mb-1")
        ...
        ui.button("Sign In", on_click=do_login).props("unelevated").classes("w-full mt-3 bg-red-700 text-white")
        ui.separator().classes("my-3")
        ui.label("Don't have an account?").classes("text-sm text-gray-500 text-center")
        ui.button("Create Account", on_click=lambda: ui.navigate.to('/register')).props("outline").classes("w-full text-red-700")
...
def register_page():
    ...
    with ui.card().classes("w-full max-w-sm mx-auto p-6 shadow-md"):
        ui.label("Create Account").classes("text-lg font-bold mb-1")
        ...
        ui.button("Create Account", on_click=do_register).props("unelevated").classes("w-full mt-3 bg-red-700 text-white")
        ui.separator().classes("my-3")
        ui.button("Back to Login", on_click=lambda: ui.navigate.to('/login')).props("outline").classes("w-full text-red-700")
...
def homepage():
    ...
    with ui.column().classes("w-full max-w-3xl mx-auto mt-6 px-4 gap-4"):
        with ui.card().classes("w-full p-6 bg-red-700 text-white shadow-md"):
            ui.label(f"Welcome back, {name}!").classes("text-h5 font-bold")
            ui.label(f"{available_count} room{'s' if available_count != 1 else ''} available right now").classes("text-sm opacity-80 mt-1")

        with ui.row().classes("w-full gap-4"):
            with ui.card().classes("flex-1 p-5 cursor-pointer hover:shadow-xl transition-shadow duration-200").on('click', lambda: ui.navigate.to('/rooms')):
                ui.icon('meeting_room', size='2rem').classes("text-red-700 mb-2")
                ui.label("Browse Rooms").classes("font-bold text-base")
                ui.label("View all available study spaces").classes("text-sm text-gray-500")

            with ui.card().classes("flex-1 p-5 cursor-pointer hover:shadow-xl transition-shadow duration-200").on('click', lambda: ui.navigate.to('/reserve')):
                ui.icon('event', size='2rem').classes("text-red-700 mb-2")
                ui.label("Make a Reservation").classes("font-bold text-base")
                ui.label("Book a room for your session").classes("text-sm text-gray-500")

            with ui.card().classes("flex-1 p-5 cursor-pointer hover:shadow-xl transition-shadow duration-200").on('click', lambda: ui.navigate.to('/reservations')):
                ui.icon('list_alt', size='2rem').classes("text-red-700 mb-2")
                ui.label("My Reservations").classes("font-bold text-base")
                ui.label("View and track your bookings").classes("text-sm text-gray-500")

        if current_role() == 'admin':
            with ui.card().classes("w-full p-4 cursor-pointer hover:shadow-xl transition-shadow duration-200 border-l-4 border-red-900").on('click', lambda: ui.navigate.to('/admin')):
                with ui.row().classes("items-center gap-3"):
                    ui.icon('admin_panel_settings', size='1.5rem').classes("text-red-700")
                    with ui.column().classes("gap-0"):
                        ui.label("Admin Panel").classes("font-bold")
                        ui.label("Review and manage pending reservations").classes("text-sm text-gray-500")
...
def rooms_page():
    ...
    ui.label("All Rooms").classes("text-h5 font-bold px-2")
    ui.label("Click Reserve to check availability and book a time.").classes("text-sm text-gray-500 px-2 mb-3")

    rooms = get_rooms()
    booked_ids = get_booked_room_ids()

    search_input = ui.input(placeholder="Search by room name or building...").props("outlined clearable").classes("w-full max-w-4xl mx-auto mb-1")

    room_container = ui.column().classes("w-full max-w-4xl mx-auto gap-4 mt-2")
    ...
    def render_rooms(filter_text=""):
        ...
        with room_container:
            ...
            for room in filtered:
                ...
                border = "border-l-4 border-red-400" if is_booked else "border-l-4 border-green-500"
                with ui.card().classes(f"w-full p-4 {border} hover:shadow-lg transition-shadow duration-200"):
                    with ui.row().classes("w-full items-center justify-between"):
                        with ui.column():
                            ui.label(room['room_name']).classes("text-base font-bold")
                            ui.label(f"Building: {room['building_name']}").classes("text-sm text-gray-600")
                            ui.label(f"Capacity: {room['capacity']}").classes("text-sm text-gray-600")
                            ui.label(f"Features: {room['features']}").classes("text-sm text-gray-600")
                            if is_booked:
                                ui.badge("Currently Unavailable", color="red").classes("mt-1")
                            else:
                                ui.badge("Available Now", color="green").classes("mt-1")
                        if is_booked:
                            ui.button("UNAVAILABLE", color="red").props("disabled unelevated")
                        else:
                            ui.button(
                                "RESERVE",
    ...
    with ui.row().classes("justify-center mt-6"):
        ui.button("Back Home", on_click=lambda: ui.navigate.to("/")).props("outline").classes("text-gray-700 w-48")
...
def reservations_page():
    ...
    if current_role() == 'admin':
        ui.label("All Reservations").classes("text-h5 font-bold mb-3")
        columns = [
        ...
        ui.label("My Reservations").classes("text-h5 font-bold mb-3")
        columns = [
            {'name': 'reservation_id', 'field': 'reservation_id', 'label': 'Reservation ID'},
            {'name': 'room_name', 'field': 'room_name', 'label': 'Room'},
            {'name': 'start_datetime', 'field': 'start_datetime', 'label': 'Start'},
    ...
    ui.label("Room Usage Summary").classes("text-h6 font-bold mb-2")
    stats_columns = [
        {'name': 'room_name', 'field': 'room_name', 'label': 'Room'},
    ...
    ui.button("Back Home", on_click=lambda: ui.navigate.to("/")).props("outline").classes("text-gray-700 w-48 mt-4")
...
def reserve_page():
    ...
    with ui.row().classes("w-full max-w-lg mx-auto gap-2 mb-4 items-center px-1"):
        s1 = ui.label("1. Select Time").classes("text-sm font-bold text-red-700 bg-red-50 px-3 py-1 rounded-full")
        ui.label("→").classes("text-gray-300 text-xs")
        s2 = ui.label("2. Choose Room").classes("text-sm text-gray-400 px-3 py-1 rounded-full")
        ui.label("→").classes("text-gray-300 text-xs")
        s3 = ui.label("3. Confirmed").classes("text-sm text-gray-400 px-3 py-1 rounded-full")

    with ui.card().classes("w-full max-w-lg mx-auto p-6 shadow-md") as step1_card:
        ui.label("Make a Reservation").classes("text-h6 font-bold mb-2")
        if room_id:
            ui.label(f"Pre-selected Room ID: {room_id}").classes("text-green-600 text-sm font-semibold")
        ...
        with ui.row().classes("gap-3 mt-4"):
            ui.button("Go Back", on_click=lambda: ui.navigate.to("/rooms")).props("outline").classes("text-gray-700")
            ui.button("Search Available Rooms", on_click=lambda: process_step1()).props("unelevated color=green")

    with ui.card().classes("w-full max-w-4xl mx-auto p-6 shadow-md") as step2_card:
        ui.label("Available Rooms").classes("text-h6 font-bold mb-1")
        ...
        time_range_label = ui.label("").classes("text-sm text-gray-500 mt-2")

        with ui.row().classes("gap-3 mt-4"):
            def go_back():
                s1.classes(add="text-red-700 bg-red-50 font-bold", remove="text-green-700 bg-green-50")
                s2.classes(add="text-gray-400", remove="text-red-700 bg-red-50 font-bold")
                step1_card.set_visibility(True)
                step2_card.set_visibility(False)
            ui.button("← Back", on_click=go_back).props("outline").classes("text-gray-700")
            ui.button("Reserve Selected Room", on_click=lambda: process_step2()).props("unelevated color=green")

    with ui.card().classes("w-full max-w-lg mx-auto p-6 shadow-md text-center") as step3_card:
        ui.label("Reservation Submitted!").classes("text-h6 font-bold text-green-700 mb-1")
        ui.label("Your reservation is now pending approval.").classes("text-sm text-gray-500 mb-4")
        with ui.row().classes("justify-center gap-3"):
            ui.button("View Reservations", on_click=lambda: ui.navigate.to("/reservations")).props("unelevated color=green")
            ui.button("Back Home", on_click=lambda: ui.navigate.to("/")).props("outline").classes("text-gray-700")
    ...
    def process_step1():
        ...
        s1.classes(add="text-green-700 bg-green-50", remove="text-red-700 bg-red-50")
        s2.classes(add="text-red-700 bg-red-50 font-bold", remove="text-gray-400")
    ...
    def process_step2():
        ...
        if success:
            s2.classes(add="text-green-700 bg-green-50", remove="text-red-700 bg-red-50")
            s3.classes(add="text-green-700 bg-green-50 font-bold", remove="text-gray-400")
            step2_card.set_visibility(False)
...
def admin_page():
    ...
    if current_role() != 'admin':
        ...
        ui.button("Back Home", on_click=lambda: ui.navigate.to("/")).props("outline").classes("text-gray-700 mt-4")
        return
    ...
    with ui.row().classes("items-center gap-3 mb-1"):
        ui.label("Admin Panel").classes("text-h5 font-bold")
        count_badge = ui.badge("0", color="red")
    ui.label("Review and action pending reservations below.").classes("text-sm text-gray-500 mb-4")
    ...
    def refresh():
        ...
        with table_container:
            if not pending:
                ui.label("No pending reservations.").classes("text-gray-400 text-center mt-8")
                return
            for res in pending:
                with ui.card().classes("w-full p-4 border-l-4 border-orange-400"):
                    with ui.row().classes("w-full items-center justify-between"):
                        with ui.column():
                            ui.label(f"Reservation #{res['reservation_id']}").classes("font-bold text-base")
                            ui.label(f"User: {res['user_name']}").classes("text-sm text-gray-600")
                            ui.label(f"Room: {res['room_name']}").classes("text-sm text-gray-600")
                            ui.label(f"From: {res['start_datetime'].strftime('%b %d, %Y %I:%M %p')}").classes("text-sm text-gray-600")
                            ui.label(f"To:   {res['end_datetime'].strftime('%b %d, %Y %I:%M %p')}").classes("text-sm text-gray-600")
                        with ui.row().classes("gap-2"):
                            rid = res['reservation_id']
                            ui.button("Approve", on_click=lambda r=rid: (approve_reservation(r, admin_id), refresh())).props("unelevated color=green")
                            ui.button("Reject",  on_click=lambda r=rid: (reject_reservation(r, admin_id),  refresh())).props("unelevated color=red")

    refresh()

    ui.button("Back Home", on_click=lambda: ui.navigate.to("/")).props("outline").classes("text-gray-700 w-48 mt-6")
